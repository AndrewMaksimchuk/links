...
const Tag: FC<{ name: string; color?: string }> = (props) => {
  const TagStyle: Style = {
    ...
    border:
      "solid 1px " + (props.color ? props.color : "var(--pico-border-color)"),
    borderRadius: "1.25em",
...
const ButtonDelete: FC<{ link_id: number; onclick: stringifiedFunction }> = (
  props
) => {
  return (
    <button
...
const ButtonShare: FC<{ title: string; text: string; url: string }> = (
  props
) => {
  const functionShareLink =
    "shareLink() { navigator.share && navigator.share({title: this.title, text: this.text, url: this.url}) }";
  const xData = `{ title: '${props.title}', text: '${props.text}', url: '${props.url}', ${functionShareLink} }`;
  return (
    <div x-data="{canShare: Boolean(navigator.share)}">
    ...
    <article id={"l" + String(props.link.link_id)} style={articleStyle}>
      ...
      <footer>
        <p style="text-align: right;">
          {new Date(props.link.created_at ?? Date.now()).toDateString()}
        </p>
        <p style="text-align: right;">
          {"string" === typeof props.link.name ? (
        ...
        <p style={LinkItemStyleButtonGroup}>
          ...
          />
          <ButtonShare text="" title={title} url={props.link.url ?? ""} />
        </p>
      </footer>
    </article>
  );
...
const LinkItemTableItem: FC<{ link: LinkItemProps }> = (props) => {
  return (
    <tr id={"l" + String(props.link.link_id)}>
      <th scope="row" style="max-width: 30ch;">
        {props.link.title || "-"}
      </th>
...
export const Links: FC<{ view: View }> = (props) => {
  ...
  const ViewOfLinkItem =
    props.view === "table" ? LinkItemTableItem : LinkItemCard;
  const RenderedLinkItem = links.map((linkData) => (
    <ViewOfLinkItem link={linkData}></ViewOfLinkItem>
  ));
  const View =
    props.view === "table" ? (
      <LinkItemTable>{RenderedLinkItem}</LinkItemTable>
    ) : (
      <LinkItemCardContainer>{RenderedLinkItem}</LinkItemCardContainer>
    );
...
const LinkFormEditButtonUpdate: FC<{ style?: Record<string, string> }> = (
  props
) => {
  return (
    <button
