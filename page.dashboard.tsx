...
export const SelectTags: FC<{
  ...
  const View = tags.length ? (
    <Fragment>
      <label for="tags" style={props.styleLabel}>
        Select tags
      </label>
      <select
        id="tags"
      ...
      >
        {undefined === props.selectedTag ? (
          <option disabled selected value={NaN}>
            Select one:
          </option>
        ) : null}
        {tags.map((tag) => (
          <option
            value={tag.tag_id}
            selected={tag.tag_id === props.selectedTag}
          >
            {tag.name}
          </option>
        ))}
  ...
  return (
    ...
    >
      ...
      <input
        ...
        minlength={12}
      />
      <small id="link-add-aria-description">
        Write or paste link with https protocol, example:
        https://github.com/AndrewMaksimchuk/links
      </small>
      <SelectTags></SelectTags>
      <button id="linkAddSubmitButton" type="submit">
        Add new link
      </button>
    </form>
  );
    ...
    <Fragment>
      ...
      >
        <input
          ...
          autofocus
        />
        <button id="searchFormSubmitButton" type="submit">
          Search
        </button>
      </form>
...
const Logout = () => {
  return (
    <form action={routes.logout} method="post" style="margin: 0;">
      <button type="submit" className="outline" style="margin: 0;">
        Logout
      </button>
    </form>
  );
...
const onClickToggleSettingsView =
  "settingsView.toggleAttribute('open'); document.body.classList.toggle('modal-is-open');";
...
const SettingsTags = () => {
  ...
  return (
    <div>
      <form
        hx-post={routes.tagCreate}
        hx-target="#createTagResponse"
        hx-swap="afterbegin"
        hx-on--after-request="this.reset()"
      >
        <fieldset>
          <legend style="text-align: center; width: 100%; font-weight: bold;">
            Create new tag
          </legend>
      ...
      <p
        ...
        style="display: flex; flex-wrap: wrap; gap: 1rem; min-height: 2rem;"
      >
        {tags.map((tag) => (
          <TagNotification
            name={tag.name}
  ...
  return (
    <dialog id="settingsView">
      <article>
        ...
        <SettingsTags />
        <footer>
          <a href={routes.userDataDownload}>Download your data</a>
        </footer>
      </article>
    </dialog>
  );
...
export const Greeting: FC<{ name: string | undefined }> = (props) => {
  ...
  return (
    <h1 id="userGreeting" style="margin: 0;">
...
export const Dashboard = (
  props: PropsWithChildren<{ user: User; linkViewState: View; tags: Tag[] }>
) => {
  return (
    <Fragment>
